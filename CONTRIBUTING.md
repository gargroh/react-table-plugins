...
- Link `react-table-plugins` locally by running `$ yarn link`
...
- To run examples using your local build, link to the local `react-table-plugins` by running `$ yarn link react-table-plugins` from the example's directory
...
`react-table-plugins` is using [Angular Commit Message Conventions](https://github.com/angular/angular.js/blob/master/DEVELOPERS.md#-git-commit-guidelines).
...
The scope could be anything specifying place of the commit change. For example `useExportData`, `useColumnSummary` etc...
